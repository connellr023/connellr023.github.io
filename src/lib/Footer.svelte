<script lang="ts">
  ...
  const apiEndpoint =
    "https://api.github.com/repos/connellr023/connellr023.github.io/commits/main";
...
<footer>
  <span class="license">
    ...
    <span class="token-yellow qualifier-after">Licenses</span><a
      class="token-yellow"
  ...
  <span class="commit">
    ...
    {:else}
      <span class="token-yellow">Some</span>{"("}<a
        class="token-pink"
        href={commit.html_url}
        target="_blank">{commitIdString}</a
      >{");"}
    {/if}
  </span>
</footer>
